...
// Skip 3D entirely on mobile — canvas is hidden via CSS anyway
if (window.innerWidth <= 768) throw new Error('mobile: skipping 3D');
...
let computers = null;
let computersBaseX = 0;
let computersBaseY = 0;
...
loader.load('assets/model/computers/scene.gltf', (gltf) => {
  ...
  computers.position.x -= 0;
  computers.position.y -= 0;
  computersBaseX = computers.position.x;
  computersBaseY = computers.position.y;
  computers.rotation.y = 0;
  computersBaseRotY = computers.rotation.y;
...
// Router calls navigate() — hook into history to detect page changes
...
function onRouteChange() {
  ...
  setTimeout(() => {
    ...
    const target = currentPage === '/projects' ? '0.18' : '1';
    canvas.style.opacity = target;
  }, 180);
}
...
(function animate() {
  ...
  if (girl && girl.visible) {
    ...
    girl.position.x = girlBaseX + smoothScrollY * 0.005;
